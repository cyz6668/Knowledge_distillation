...
teacher = torch.load('/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/non_vehicle_multiclass/trained_models2/non_vehicle_3_10_teacher.pth')
student=ResNet18()
...
criterion_f = betweenLoss([0.001,0.01,0.05,0.1,1], loss=CrossEntropy)

def train_and_valid(teacher,model, loss_function, optimizer, epochs=25):
    ...
    for epoch in range(epochs):#训练epochs轮
        ...
        for i, data in enumerate(train_data):
            ...
            outputs2,tea_f=teacher(inputs)
            out_result = torch.softmax(outputs, dim=1)
            out_result=out_result.reshape(-1,5)
            out_result2 = torch.softmax(outputs2, dim=1)
            out_result2=out_result2.reshape(-1,5)

            
            output, answer = output_selector(stu_f, tea_f, [0,1,2,3,4])
            loss = criterion_f(output, answer)
     
            loss_lambda=0.5
            temperature=3
            # batch_y_teacher = torch.softmax(outputs2 / temperature, dim=1).detach()
            # loss = (1 - loss_lambda) * criterion[0](out_result, labels) + \
            #     loss_lambda * temperature**2 * criterion[1](torch.softmax(outputs / temperature, dim=1), batch_y_teacher)
            # assert not torch.isnan(loss)

            # out_result=outputs.reshape(-1,5)
            # #3.14 Exploring Knowledge distillation of DNN for efficient hardware solutions
            # loss=nn.KLDivLoss()(F.log_softmax(outputs/temperature,dim=1),F.softmax(outputs2/temperature,dim=1))*(0.5*temperature*temperature)+F.cross_entropy(out_result,labels)*(1.-0.5)

            # loss1=CrossEntropy(outputs,outputs2)
            ...
            _,predictions2=out_result2.max(1)
            relative_acc+=predictions.eq(predictions2).sum().item()
            train_acc += predictions.eq(labels).sum().item()
            print(str(i)+"/"+str(len(train_data))+"  G_loss:"+str(train_loss / (i + 1))+"  acc:"+str( 100. * train_acc / total)+"  | "+str(train_acc)+"/"+str(total)+"relative  acc:"+str( 100. * relative_acc / total)+"  | "+str(relative_acc)+"/"+str(total))
        ...
        torch.save(model, '/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/KD_feature_student_model_' + str(epoch + 1)+ '_'+str(format(avg_train_acc * 100,'.2f'))+'_'+str(format(avg_train_loss * 100,'.2f'))+ '.pth')
    f=open("/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/rec_feature.txt",'a')
    f.write("\n")
    lens=len(data_rec)
    for i in range(lens):
        f.write(data_rec[i])
...
if __name__=='__main__':
    num_epochs = 50
    trained_model, record = train_and_valid(teacher,student, loss_func, optimizer, num_epochs)
