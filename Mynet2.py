...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes, kernel_size=1, stride=2, bias=False),
                nn.BatchNorm2d(self.expansion*planes)
            )
...
class ResNet(nn.Module):
    def __init__(self,  block, num_blocks, num_classes=5):
        ...
        self.final=nn.Conv2d(512*block.expansion,num_classes,kernel_size=1,stride=1)

        self.fc_out =1
        self.out_dims = [5000,1000,500,200,10]
        if self.fc_out:
...
def test():
    ...
    # args = {"fc_out":1,"pool_out":"avg","out_layer": [0,1,2,3,4],"out_dims":[10000,5000,1000,500,5]}
    net = ResNet50()
    
    checkpoint=torch.load('/mnt/cfs/users/yuezi.chen/MEAL-master2/resnet50_pre.pth')
