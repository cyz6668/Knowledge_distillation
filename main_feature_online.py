...
def train_and_valid(teacher,model, loss_function, optimizer, epochs=25):
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")#若有gpu可用则用gpu
    record = []
    best_acc = 0.0
    best_epoch = 0
    data_rec=[]
    ...
    for epoch in range(epochs):#训练epochs轮
        ...
        for i, data in enumerate(train_data):
            ...
            loss2=F.cross_entropy(out_result2,labels)
            ...
            tea_acc+=predictions2.eq(labels).sum().item()
            train_acc += predictions.eq(labels).sum().item()
            print(str(i)+"/"+str(len(train_data))+"  G_loss:"+str(train_loss / (i + 1))+"  acc:"+str( 100. * train_acc / total)+"  | "+str(train_acc)+"/"+str(total)+"| relative  acc:"+str( 100. * relative_acc / total)+"  | "+str(relative_acc)+"/"+str(total)+"| teacher acc:"+str( 100. * tea_acc / total)+"  | "+str(tea_acc)+"/"+str(total))
        ...
        torch.save(model, '/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/KD_feature_online_student_model_' + str(epoch + 1)+ '_'+str(format(avg_train_acc * 100,'.2f'))+'_'+str(format(avg_train_loss * 100,'.2f'))+ '.pth')
    f=open("/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/rec_feature_online.txt",'a')
    f.write("\n")
    lens=len(data_rec)
    for i in range(lens):
        f.write(data_rec[i])
