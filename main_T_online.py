...
def train_and_valid(teacher,model, loss_function, optimizer, epochs=25):
    device = torch.device("cuda:4" if torch.cuda.is_available() else "cpu")#若有gpu可用则用gpu
    record = []
    best_acc = 0.0
    best_epoch = 0
    data_rec=[]
    ...
    for epoch in range(epochs):#训练epochs轮
        ...
        for i, data in enumerate(train_data):
            ...
            out_result2=outputs2.reshape(-1,5)
            loss_lambda=0.5
            temperature=3
            ...
            loss2.backward(retain_graph=True)  #为什么是读了第二次？？
        ...
        torch.save(model, '/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/KD_T_online_student_model_' + str(epoch + 1)+ '_'+str(format(avg_train_acc * 100,'.2f'))+'_'+str(format(avg_train_loss * 100,'.2f'))+ '.pth')
    f=open("/mnt/cfs/users/yuezi.chen/pytorch-retinanet-master/KD/rec_T_online.txt",'a')
    f.write("\n")
    lens=len(data_rec)
    for i in range(lens):
        f.write(data_rec[i])
